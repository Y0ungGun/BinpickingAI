...
{
    public class Inferencer : Agent
    {
        public InfSpawner Infspawner;
        ...
        public override void OnEpisodeBegin()
        {
            switch(Spawn.objectType)
            {
                case Spawn.ObjectType.Cube:
                    Infspawner.SpawnCubes(Spawn.numObjs);
                    break;
                case Spawn.ObjectType.Debug:
                    Infspawner.SpawnDebug();
                    break;
                case Spawn.ObjectType.Bolt:
                    Infspawner.SpawnBolts(Spawn.numObjs);
                    break;
            }
        }
        public override void CollectObservations(VectorSensor sensor)
        {
            ...
            target = Utils.GetTarget(Infspawner.Objects, targetXYZ.x, targetXYZ.y, targetXYZ.z);
            ...
            Debug.Log($"Current Graspability: {currentGraspability}");
        ...
        public override void OnActionReceived(ActionBuffers actions)
        {
            ...
            // y = targetXYZ.y - 0.1f;
            Debug.Log($"Action Received: x:{actions.ContinuousActions[0]}, y:{actions.ContinuousActions[1]}, z:{actions.ContinuousActions[2]}, rx:{actions.ContinuousActions[3]}, ry:{actions.ContinuousActions[4]}, rz:{actions.ContinuousActions[5]}");
        ...
        public GameObject SpawnGripper(float x, float y, float z, float rx, float ry, float rz)
        {
            ...
            GameObject root = Instantiate(gripperPrefab, new Vector3(x, y, z) + rotation * Vector3.up * 0.8f, rotation, transform.parent);
            root.name = "gripper";
        ...
        void FixedUpdate()
        {
            ...
            if (controlFlag.isMoving)
            {
                ...
                timeoutReached = framesPassed >= 500;
